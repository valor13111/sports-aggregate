...
function Card({ conference, modalTarget }) {
    ...
    async function fetchData(conference) {
        ...
        .then(response => {
            ...
            result.forEach(standing => {
                standing.teamId = getTeamFullName(standing.teamId);
            });
                ...
                <div className="list-group">
                    {
                        ...
                        standings.sortedTopFiveList.map(item => 
                            <>
                                <button className="list-group-item list-group-item-action" onClick={() => setTeamDetail({data: item})} data-toggle="modal" data-target={`#${modalTarget}`}>
                                    {item.teamId} - ({item.win} / {item.loss}) 
                                </button>

                                <TeamDetailModal teamDetail={teamDetail.data} modalTarget={modalTarget} />
                            </>
                        )
                        : null
                    }     
